...
Uint16 uart_recv_data(char *uart_buffer)
{
   Uint16 flag = 0;
   UART_Handle  uart =(UART_Handle) &UART_MODULE_1;
  	while(C6747_UART_rcvReady(uart)==0)
  		{
		C6747_UART_getChar(uart,uart_buffer);
		//C6747_UART_putChar(uart, *uart_buffer);
		flag = 1;
  		}
